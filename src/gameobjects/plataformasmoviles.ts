...
export default class PlataformasMoviles extends Phaser.Physics.Arcade.Group {
    ...
    constructor(escena: Nivel1, nombreObjeto: string, idObjeto: 
    string, velocidad: number, horizontal: boolean) {
        ...
        Constantes.MAPAS.PLATAFORMAHORIZONTAL : 
        Constantes.MAPAS.PLATAFORMAVERTICAL;
        ...
        (nombreObjeto, {name: nombreObjetoPlataforma, key:idObjeto}));
        ...
        this.children.entries.map((plataforma: any) => {            
            ...
            if (this.horizontal){
                ...
                this.muevePlataformaHorizontal((Phaser.Math.
                Between(0, 1) ? 'izda' : 'dcha'), plataforma);
            }else{
                plataforma.body.setFrictionY(1);
                plataforma.body.setVelocityY(this.velocidad);                         
                this.muevePlataformaVertical((Phaser.Math.Between(0, 1)
                 ? 'arriba' : 'abajo'), plataforma);                
            }
    ...
    muevePlataformaHorizontal(direccion: string, enemigo: any): void {        
        (direccion === 'izda')? enemigo.body.setVelocityX
        (this.velocidad*-1):enemigo.body.setVelocityX(this.velocidad);                    
    }

    muevePlataformaVertical(direccion: string, enemigo: any): void{        
        (direccion === 'arriba')? enemigo.body.setVelocityY
        (this.velocidad*-1):enemigo.body.setVelocityY(this.velocidad);            
    }
        ...
        this.children.entries.map((enemigo: any) => {
            if (this.horizontal){
                if(enemigo.body.velocity.x === 0) {
                    ...
                    Phaser.Math.Between(0, 1) ? 'izda' : 'dcha'), enemigo);
                }
                ...
                if(enemigo.body.velocity.y === 0) {
                    this.muevePlataformaVertical((Phaser.Math.
                    Between(0, 1) ? 'arriba' : 'abajo'), enemigo);
                }
